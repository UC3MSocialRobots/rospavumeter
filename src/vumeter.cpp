...
static void context_get_server_info_callback(pa_context *c, const pa_server_info*si, void *) {
  ...
  if (mode == PLAYBACK) {
    ...
    g_message("default_source_name:%s", si->default_source_name);
...
int main(int argc, char *argv[]) {
  ...
  bool record = false;
  int device_name_idx = -1;
  nh_private.param("record", record, record);
  nh_private.param("device_name", device_name_idx, device_name_idx);
  ROS_INFO("device_name_idx:%i", device_name_idx);
  if (device_name_idx >= 0) {
    std::ostringstream out; out << device_name_idx;
    device_name = g_strdup(out.str().c_str());
  }
